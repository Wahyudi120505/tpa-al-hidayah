...
const GradeStudent = () => {
  ...
  const [selectedGrade, setSelectedGrade] = useState(null); // State for selected grade
          ...
          <div className="p-8">
            {filteredGrades.length > 0 ? (
              <div className="space-y-6">
                {filteredGrades.map((grade) => (
                  <div
                    ...
                    onClick={() => handleCardClick(grade)} // Add click handler
                  >
              ...
              <div className="text-center text-gray-500 py-12 flex flex-col items-center gap-3">
                ...
                <p>No grades available for this parent.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full mx-4 relative transform transition-all duration-300 scale-100">
            ...
            <div className="space-y-4">
              <p className="text-gray-700 flex items-center gap-2">
                ...
                <span className="font-medium">Subject:</span> {selectedGrade.name}
              </p>
              <p className="text-gray-700 flex items-center gap-2">
                <User className="w-5 h-5 text-indigo-500" />
                <span className="font-medium">Student:</span>{" "}
              ...
              <p className="text-gray-700 flex items-center gap-2">
                ...
                {selectedGrade.responseStudent.gender === "L" ? "Male" : "Female"}
              </p>
              <p className="text-gray-700 flex items-center gap-2">
                <Calendar className="w-5 h-5 text-indigo-500" />
                <span className="font-medium">Birth Date:</span>{" "}
                {new Date(selectedGrade.responseStudent.birthDate).toLocaleDateString()}
              </p>
              <p className="text-gray-700 flex items-center gap-2">
                <User className="w-5 h-5 text-indigo-500" />
              ...
              <p className="text-gray-700 flex items-center gap-2">
                <Calendar className="w-5 h-5 text-indigo-500" />
                <span className="font-medium">Semester:</span> {selectedGrade.semester}
              </p>
            </div>
            <button
              onClick={closeModal}
